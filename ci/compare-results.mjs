...
function collectRecords(dir) {
  ...
  // CLI flows: the cast reports the capture wrote (`casts/*.report.json`, or
  // copied flat into the package). Keyed `cli:<flow id>` so a scaffold that
  // stops answering a prompt, or an install that starts failing, is compared
  // exactly like a page. A CLI-only package is always partial: pages absent
  // from it were not run, not lost.
  const castDir = fs.existsSync(path.join(dir, 'casts')) ? path.join(dir, 'casts') : dir;
  const casts = fs.existsSync(castDir) ? fs.readdirSync(castDir).filter((f) => f.endsWith('.report.json')) : [];
  if (casts.length && !raw.length && !flat.length) partial = true;
  for (const f of casts) {
    const rep = readJson(path.join(castDir, f));
    if (!rep.id) continue;
    sources.push(f);
    byId.set(`cli:${rep.id}`, {
      id: `cli:${rep.id}`,
      name: rep.name ?? rep.id,
      filename: rep.castFile ?? '',
      success: rep.success !== false,
      error: rep.success === false ? cliError(rep) : undefined,
      warnings: rep.missingFiles?.length ? [`missing: ${rep.missingFiles.join(', ')}`] : [],
      source: f,
    });
  }
...
/** "Exited 1, expected 0." says nothing; the last error-like line on screen does. */
function cliError(rep) {
  const tail = String(rep.tail ?? '').split('\n').map((l) => l.trim()).filter(Boolean);
  const hit = [...tail].reverse().find((l) => /error|not found|failed|cannot|ERR_|refused|denied/i.test(l));
  const code = rep.exitCode != null ? `[CLI Exit ${rep.exitCode}]` : '[CLI Failure]';
  return `${code} ${hit ?? rep.error ?? 'failed'}`;
}
